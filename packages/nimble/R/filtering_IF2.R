...
doPars1 <- nimbleFunction(
  ...
  methods = list(
    ...
    calcScalarMean = function(m = integer()){
      ...
      sumMean = 0
      for(i in 1:m){
        sumMean <- sumMean + mvWSamples[parName, i][1]*
          exp(mvWSamples['wts', i][1])
      }
      return(sumMean)
    },
    calcVectorMean = function(m = integer(), length = integer()){
      ...
      for(i in 1:m){
        sumMean[1:length] <- sumMean[1:length] + 
          mvWSamples[parName, i][1:length]*exp(mvWSamples['wts', i][1])
      }
      return(sumMean)
    }
...
IF2Step <- nimbleFunction(
  ...
  run = function(m = integer(), j = integer(), coolingRate = double(), useStoredSamples = integer()) {
    ...
    coolSigma <- coolParam*sigmaMat
    coolChol <- chol(coolSigma)
    for(i in 1:m) {
    ...
    for(i in 1:m) {
       if(useStoredSamples == 1) {
         if(singleParam == 1){
           tmpPars[1,i] <- rnorm(1, tmpPars[1,i],  coolSigma[1,1])
           values(model, paramNodes) <<- tmpPars[,i]
         }
         else{
           tmpPars[,i]  <-  rmnorm_chol(1, tmpPars[,i],  coolChol[,], prec_param=0)
           values(model, paramNodes) <<- tmpPars[,i] 
         }
         if(notFirst){
         ...
         for(j in 1:numParamVars){
           if(varSize[j] == 1){
             tmpPars[paramInds[j+1], i] <- rnorm(1, values(model, paramVars[j])[1],
                                                 coolSigma[paramInds[j+1],paramInds[j+1]])
           }
           else{
             tmpPars[(paramInds[j]+1):paramInds[j+1], i] <- rmnorm_chol(1,
                                                                        values(model, paramVars[j]),
                                                                        coolSigma[,], prec_param=0)
           }
         }
         values(model, paramNodes) <<- tmpPars[,i]
      }
...
#' 
#' @description Create a If2 algorithm for a given NIMBLE state space model.  
#'
...
#' \describe{
#'  \item{sigma}{A vector specifying a non-negative preturbation magnitude for each element of the \code{params} argument.  Defaults to a vector of 1's.}
...
#'
#'  The \code{run} fuction will return a vector with MLE estimates.  Additionally, once the specialized algorithm has been run, it can be continued for additional iterations by calling the continueRun method.
#'  
#' @export
#' 
...
buildIteratedFilter2 <- nimbleFunction(
  setup = function(model, nodes, params = NULL, control = list()){
    ...
    if(length(sigma) < paramsLength){
      stop("the 'sigma' control list argument must be a vector specifying a
           non-negative preturbation magnitude for each element of
           the `params` argument.  The provided 'sigma' value had an incorrect
           length.") 
    }
    if(any(sigma < 0)){
      stop("the 'sigma' control list argument must be a vector specifying a
           non-negative preturbation magnitude for each element of
           the `params` argument.  The provided 'sigma' value had negative 
           elements.") 
    }
  ...
  run = function(m = integer(default = 10000), n = integer(default = 5),
    ...
    originalParamVals <- values(model, params)
    ...
    oldM <<- m
    oldJ <<- j + 1
    values(model, params) <<-  originalParamVals 
    
    for(i in 1:paramsLength)
      sortedOutPars[i] <- outPars[sortingOrder[i]]
